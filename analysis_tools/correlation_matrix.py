...
import numpy as np
import itertools
from pil_tools import text_manipulation


def correlation_matrix_generator(asset_names, period='1y', visual=True, img_name='correlation_matrix', interval='1d'):
    """
    :param asset_names: List of the assets to be correlated.
    ...
    :param img_name: Saved image name, defaults to correlation_matrix.
    :param interval: Interval
    :return: Correlation matrix as a pandas dataframe.
    """

    correlation_matrix = pd.DataFrame(columns=asset_names, index=asset_names)
    assets = []
